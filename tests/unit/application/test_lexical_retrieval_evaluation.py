from pathlib import Path

import pytest

import knowledge_base_assistant.application.lexical_retrieval_evaluation as module
from knowledge_base_assistant.domain.models import Chunk
from knowledge_base_assistant.evaluation.models import (
    GoldenQuery,
    QueryEvaluationResult,
    RetrievalEvaluationResult,
)


class FakeBM25Index:
    pass


def test_evaluate_bm25_retrieval_builds_index_and_calls_common_evaluator(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    golden_path = Path("golden.jsonl")
    chunks_path = Path("chunks.jsonl")

    queries = [
        GoldenQuery(
            query_id="query-1",
            query="test query",
            relevant_chunks=(),
            notes="",
        ),
    ]
    chunks = [_make_chunk()]
    fake_index = FakeBM25Index()

    monkeypatch.setattr(
        module,
        "read_golden_queries_jsonl",
        lambda path: queries,
    )
    monkeypatch.setattr(
        module,
        "read_chunks_jsonl",
        lambda path: chunks,
    )

    build_calls: list[tuple[list[Chunk], float, float]] = []

    def fake_build(
        received_chunks: list[Chunk],
        *,
        k1: float,
        b: float,
    ) -> FakeBM25Index:
        build_calls.append((received_chunks, k1, b))
        return fake_index

    monkeypatch.setattr(
        module.BM25Index,
        "build",
        fake_build,
    )

    expected_result = RetrievalEvaluationResult(
        top_k=5,
        query_count=1,
        evaluated_query_count=0,
        no_answer_query_count=1,
        hit_rate_at_k=0.0,
        recall_at_k=0.0,
        mean_reciprocal_rank=0.0,
        ndcg_at_k=0.0,
    )

    evaluator_calls: list[dict[str, object]] = []

    def fake_evaluate_retrieval(
        *,
        queries,
        chunks,
        retriever,
        top_k,
    ) -> RetrievalEvaluationResult:
        evaluator_calls.append(
            {
                "queries": queries,
                "chunks": chunks,
                "retriever": retriever,
                "top_k": top_k,
            }
        )
        return expected_result

    monkeypatch.setattr(
        module,
        "evaluate_retrieval",
        fake_evaluate_retrieval,
    )

    result = module.evaluate_bm25_retrieval(
        golden_path=golden_path,
        chunks_path=chunks_path,
        top_k=5,
        k1=1.2,
        b=0.6,
    )

    assert result == expected_result
    assert build_calls == [
        (chunks, 1.2, 0.6),
    ]
    assert evaluator_calls == [
        {
            "queries": queries,
            "chunks": chunks,
            "retriever": fake_index,
            "top_k": 5,
        },
    ]


def _make_chunk() -> Chunk:
    return Chunk(
        chunk_id="chunk-1",
        document_id="document-1",
        source_name="test-source",
        relative_path="notes/test.md",
        title="Test",
        section_path=("Test",),
        content="Content",
        searchable_text="Searchable content",
        chunk_index=0,
        section_chunk_index=0,
        start_line=1,
        end_line=2,
        content_hash="content-hash",
    )
    
    
def test_evaluate_bm25_queries_builds_index_and_calls_common_evaluator(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    golden_path = Path("golden.jsonl")
    chunks_path = Path("chunks.jsonl")

    queries = [
        GoldenQuery(
            query_id="query-1",
            query="test query",
            relevant_chunks=(),
            notes="",
        ),
    ]
    chunks = [_make_chunk()]
    fake_index = FakeBM25Index()
    expected_result: tuple[QueryEvaluationResult, ...] = ()

    monkeypatch.setattr(
        module,
        "read_golden_queries_jsonl",
        lambda path: queries,
    )
    monkeypatch.setattr(
        module,
        "read_chunks_jsonl",
        lambda path: chunks,
    )
    monkeypatch.setattr(
        module.BM25Index,
        "build",
        lambda received_chunks, *, k1, b: fake_index,
    )

    evaluator_calls: list[dict[str, object]] = []

    def fake_evaluate_queries(
        *,
        queries,
        chunks,
        retriever,
        top_k,
    ) -> tuple[QueryEvaluationResult, ...]:
        evaluator_calls.append(
            {
                "queries": queries,
                "chunks": chunks,
                "retriever": retriever,
                "top_k": top_k,
            }
        )
        return expected_result

    monkeypatch.setattr(
        module,
        "evaluate_queries",
        fake_evaluate_queries,
    )

    result = module.evaluate_bm25_queries(
        golden_path=golden_path,
        chunks_path=chunks_path,
        top_k=5,
        k1=1.2,
        b=0.6,
    )

    assert result == expected_result
    assert evaluator_calls == [
        {
            "queries": queries,
            "chunks": chunks,
            "retriever": fake_index,
            "top_k": 5,
        },
    ]